...
//const int INODE_COUNT = (int *) ceil((255 * sizeof(inode_t)) / 4096);
...
void blocks_init(const char *image_path) {
	...
	// if block 1 is free, then inode table has not been initialized
	// inode table starts after bitmaps and continues into block 1
	if (!bitmap_get(bbm, 1)) {
		alloc_block();
		bitmap_put(bbm, 1, 1);
	}
